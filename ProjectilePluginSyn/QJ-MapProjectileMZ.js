...
/*:
 * @target MV MZ
 * @plugindesc [Projectile In Map MZ][V2.0]
 * @author Qiu Jiu
 * @base QJ-CoreMZ
 *
...
Game_QJBulletMZ.prototype.refreshBox = function(justMakeData) {
    ...
    if (this.data.collisionBox[0]=='auto') {
        ...
        body = justMakeData?justMakeData:this.data.collisionBox;
    }
    let newSATBody;
...
Game_QJBulletMZ.prototype.JudgeReBound = function (sat,tb,cb,detail) {
    if (!detail.rb||detail.rb[0]==0) {
        ...
        this.remData();
        QJ.SAT.setPostion(this.QJBody,this.x,this.y);
        //====================================
...
Game_QJBulletMZ.prototype.updateExistData = function(haveTile = true,haveRebound = true,haveTime = true,ifPierce = true,ifTarget = true) {
    ...
    if (this.needBody) {
        startX = Math.max(Math.min(Math.floor((this.x-this.QJBody.dia)/tileSize),$gameMap.width()-1),0);
        startY = Math.max(Math.min(Math.floor((this.y-this.QJBody.dia)/tileSize),$gameMap.height()-1),0);
        endX = Math.max(Math.min(Math.ceil((this.x+this.QJBody.dia)/tileSize),$gameMap.width()),0);
        endY = Math.max(Math.min(Math.ceil((this.y+this.QJBody.dia)/tileSize),$gameMap.height()),0);
    }
    ...
    for (let k=1,kdata=this.data.existData,kl=kdata.length,detail;k<kl;k++) {
                ...
                for (let x=startX;x<endX;x++) {
                    for (let y=startY;y<endY;y++) {
                        ...
                        if (detail.t[1].includes(gridData)) {
                            ...
                            QJ.SAT.judge(this.QJBody,mapBox);
                            if (haveRebound) {
                                if (result.result&&this.JudgeReBound(result,this.QJBody,mapBox,detail)) return;
                            } else {
        ...
        if (ifTarget) {
            if (detail.t[0]=='P') {
                //=============================================================
                QJ.SAT.judge(this.QJBody,$gamePlayer._boxBodyQJ);
                if (haveRebound) {
                    if (result.result&&this.JudgeReBound(result,this.QJBody,$gamePlayer._boxBodyQJ,detail)) {
                ...
                for (let i of detail.t[1]) {
                    if ($gameMap._groupListQJ[i]) {
                         for (let j of $gameMap._groupListQJ[i]) {
                            ...
                            character = QJ.getCharacter(j);
                            if (character&&character._boxBodyQJ) {
                                QJ.SAT.judge(this.QJBody,character._boxBodyQJ);
                                if (haveRebound) {
                                    if (result.result&&this.JudgeReBound(result,this.QJBody,character._boxBodyQJ,detail)) {
                         ...
                         for (let j in $gameMap._mapBulletsNameQJ[i]) {
                            ...
                            if (character!=this&&character&&(character.QJBody||character.QJBodies)) {
                                ...
                                for (let QJBody of bodyList) {
                                    QJ.SAT.judge(this.QJBody,QJBody);
                                    if (haveRebound) {
                                        if (result.result&&this.JudgeReBound(result,this.QJBody,QJBody,detail)) {
